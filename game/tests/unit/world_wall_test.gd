...
func test_the_edge_of_the_world_is_a_wall_ten_above_sea_level() -> void:
	...
	for y in [0, 20, wall_top]:
		equal(int(data.decode_u16(WorldGen.bidx(0, y, 0))), Blocks.BEDROCK,
			"the border column is bedrock at y=%d" % y)
	equal(int(data.decode_u16(WorldGen.bidx(0, wall_top + 1, 0))), Blocks.AIR,
		"but open sky above the wall's top")
	equal(int(data.decode_u16(WorldGen.bidx(1, WorldGen.CHUNK_H - 1, 0))), Blocks.AIR,
...
func test_the_caverns_start_you_underground() -> void:
	...
	check(data.decode_u16(WorldGen.bidx(lx, spawn.y, lz)) != Blocks.AIR, "standing on ground")
	equal(int(data.decode_u16(WorldGen.bidx(lx, spawn.y + 1, lz))), Blocks.AIR, "with air over it")
